...
const KEYWORD_MAP = [
    // 1. Scene description (priority)
    { keywords: ['scan surroundings', 'scan the surroundings', 'describe surroundings', 'describe the room', 'look around', 'what do you see', 'scan room', 'what\'s around me', 'tell me what you see', 'describe my surroundings', 'what\'s in front of me', 'look around me', 'describe scene', 'scan scene', 'analyze scene', 'describe'],
                                                            skill: 'scene',      action: 'scan' },

    // 2. OCR text reading (priority)
    { keywords: ['read text', 'read this', 'scan text', 'read what\'s in front of me', 'read this page', 'read what\'s here', 'read the screen', 'ocr'],
                                                            skill: 'ocr',        action: 'read' },

    // 3. Camera open (priority)
    { keywords: ['open camera', 'launch camera', 'start camera', 'camera', 'open the camera', 'open scanner', 'start scanning'],
                                                            skill: 'camera',     action: 'open' },

    // 4. Object Finder (priority)
    { keywords: ['find', 'locate', 'where is', 'search for', 'look for'],
                                                            skill: 'objectFinder', action: 'find' },

    // 5. Emergency Calling (priority)
    { keywords: ['call emergency contact', 'call emergency', 'call helper', 'contact emergency'],
                                                            skill: 'emergency',  action: 'callContact' },
    ...
    // Camera / General Scanning Controls
    { keywords: ['start scan', 'start scanning', 'scan now', 'scan'], skill: 'camera',   action: 'startScan' },
    { keywords: ['stop scan', 'stop scanning'],             skill: 'camera',     action: 'stopScan' },
    { keywords: ['text mode', 'read mode'],                 skill: 'camera',     action: 'switch_ocr' },
    { keywords: ['scene mode', 'describe mode'],            skill: 'camera',     action: 'switch_scene' },

    // Speech controls
    { keywords: ['stop', 'stop talking', 'quiet', 'silence'], skill: 'speech',   action: 'stop' },
    ...
    // Emergency Alerts
    { keywords: ['sos', 'send sos', 'emergency', 'trigger emergency', 'send emergency alert', 'emergency alert', 'help me'],             skill: 'emergency', action: 'sendSOS' },
...
class FuzzyMatcher {
    ...
    match(rawTranscript) {
        ...
        for (const entry of KEYWORD_MAP) {
            for (const kw of entry.keywords) {
                if (normalized.includes(kw)) {
                    ...
                    return this._buildIntent(entry, rawTranscript, 0.92, normalized);
                }
            }
        }
        ...
        for (const entry of KEYWORD_MAP) {
            for (const kw of entry.keywords) {
                ...
                if (dist <= 2) {
                    ...
                    return this._buildIntent(entry, rawTranscript, 0.90, normalized);
                }
     ...
     _extractObject(normalized) {
         const match = normalized.match(/(?:find|locate|where is|search for|look for)\s+(?:my\s+|the\s+)?(.+)/);
